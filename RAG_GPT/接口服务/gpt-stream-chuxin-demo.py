...
def get_rag(texts: list):
    ...
    res = client.search(
        ...
        limit=4,  # Max. number of search results to return
...
def gpt_response():
    try:
        ...
        role_messages.append(SystemMessage(content="如果是根据System消息回答有关内容，请不要指出，另外，现在是北京时间：{}".format(beijing_time.strftime('%Y-%m-%d %H:%M:%S'))))
        ...
        for system_message in rag_contents:
            # role_messages.append(SystemMessage(content="接下来是另一个相关的文档"))
            all_rag_contents += system_message
            # role_messages.append(SystemMessage(system_message))
        role_messages.append(SystemMessage(content=all_rag_contents))
        ...
        # role_messages.append(HumanMessage(content="我一共给你提供了几条相关文档"))
        print(role_messages)
